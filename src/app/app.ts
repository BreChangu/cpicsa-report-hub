...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, FormsModule, SmartDropzoneComponent, ImageCropperComponent],
...
export class App implements OnDestroy {
  ...
  // 👇 ESTADO DEL ZOOM PARA EL CROPPER 👇
  currentZoom = signal(1);

  dynamicCategories = computed(() => {
    const cats = new Set<string>(['MANTENIMIENTO']);
    this.evidences().forEach((e) => {
      const category = this.normalizeCategory(e.category, '');
      if (category) {
        cats.add(category);
      }
    });
    return Array.from(cats).sort((a, b) => this.sortCategories(a, b));
  });

  categoryOptions = computed(() => {
    const categoryCounts = new Map<string, number>();
    this.dynamicCategories().forEach((category) => categoryCounts.set(category, 0));

    this.evidences().forEach((evidence) => {
      const category = this.normalizeCategory(evidence.category);
      categoryCounts.set(category, (categoryCounts.get(category) ?? 0) + 1);
    });

    return Array.from(categoryCounts.entries())
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => this.sortCategories(a.name, b.name));
  });
  ...
  onImageCropped(event: ImageCroppedEvent, id: string) {
    ...
    this.evidences.update((current) =>
      current.map((item) =>
        item.id === id ? { ...item, croppedBlob: event.blob ?? undefined } : item,
      ),
    );
  }
  ...
  zoomOut() {
    this.currentZoom.update((z) => Math.max(z - 0.25, 1));
  }
  ...
  unselectedCount() {
    return this.evidences().length - this.selectedCount();
  }

  hasFilterCategory() {
    return this.normalizeCategory(this.filterCategory(), '') !== '';
  }

  canApplyBulkCategory() {
    return this.hasSelectedEvidences() && this.normalizeCategory(this.bulkCategory(), '') !== '';
  }

  canApplyBulkCapturedDate() {
    return this.hasSelectedEvidences() && this.bulkCapturedDate() !== '';
  }

  clearSelection() {
    this.evidences.update((current) => current.map((item) => ({ ...item, selected: false })));
  }
  ...
  applyBulkCapturedDate() {
    ...
    if (!date) return;
    this.evidences.update((current) =>
      current.map((item) => (item.selected ? { ...item, capturedDate: date } : item)),
    );
  }

  // 👇 FIX: LA FUNCIÓN QUE FALTABA PARA QUE NO DE ERROR 👇
  selectByCategory() {
    const targetCat = this.normalizeCategory(this.filterCategory(), '');
    if (!targetCat) return;
    this.evidences.update((current) =>
      current.map((item) => ({
        ...item,
        selected: this.normalizeCategory(item.category) === targetCat,
      })),
    );
  }

  applyBulkCategory() {
    const cat = this.normalizeCategory(this.bulkCategory(), '');
    if (!cat) return;
    this.evidences.update((current) =>
      current.map((item) => (item.selected ? { ...item, category: cat } : item)),
    );
  }
      ...
      for (const category of sortedCategories) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          if (i === 0 && hasTitle) {
            ...
            pdf.text(
              `Emitido: ${this.formatCapturedDate(this.getTodayDateInputValue())}`,
              282,
              38,
              { align: 'right' },
            );
          ...
          if (itemLeft) {
            ...
            const linesRight = itemRight
              ? this.getSplitLines(pdf, itemRight, i * 2 + 1, cardWidth)
              : [];

            const maxLinesInRow = Math.max(linesLeft.length, linesRight.length);

            await this.drawElegantCard(
              pdf,
              itemLeft,
              linesLeft,
              maxLinesInRow,
  ...
  private drawElegantHeader(pdf: any) {
    pdf.setTextColor(0, 160, 227);
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(22);
    pdf.text('CPICSA', 15, 16);
    pdf.setTextColor(141, 198, 63);
    pdf.setFontSize(7.5);
    pdf.text('CONTROL DE PLAGAS INTERNACIONAL', 15, 20.5);
    pdf.text('CENTINELA, S.A. DE C.V.', 15, 23.5);
    pdf.setTextColor(71, 85, 105);
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(8);
    pdf.text('PLANTA TEMAXCAL No. 2A', 282, 15, { align: 'right' });
    pdf.text('COL. ELECTRA, C.P. 54060', 282, 18.5, { align: 'right' });
    pdf.text('TLALNEPANTLA, ESTADO DE MÉXICO', 282, 22, { align: 'right' });
    pdf.setDrawColor(226, 232, 240);
    pdf.setLineWidth(0.2);
    pdf.line(15, 27, 282, 27);
  }

  private getSplitLines(
    pdf: any,
    item: EvidenceItem,
  ...
  private async drawElegantCard(
    ...
    pdf.text(this.formatCapturedDate(item.capturedDate), x + w - 4, y + 6, { align: 'right' });
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(9);
    pdf.setTextColor(26, 34, 56);

    let displayLines = [...lines];
    if (displayLines.length > 8) {
      displayLines = displayLines.slice(0, 8);
      displayLines[7] = displayLines[7].replace(/\.+$/, '') + '...';
    }
    ...
    pdf.setDrawColor(226, 232, 240);
    pdf.line(x + 4, dynamicSeparatorY, x + w - 4, dynamicSeparatorY);
    ...
    pdf.addImage(
      dataUrl,
      this.mapMimeTypeToPdfFormat(item.reportFile.type),
      x + (w - dim.width) / 2,
      imgBoxStartY + (maxImgH - dim.height) / 2,
      dim.width,
      dim.height,
    );
  }
  ...
  private drawElegantFooter(pdf: any, pNum: number) {
    ...
    pdf.setFontSize(8);
    pdf.setTextColor(100, 116, 139);
    pdf.text('contacto@cpicsa.com', 50, fY - 3, { align: 'center' });
    pdf.text('www.cpicsa.com', 148.5, fY - 3, { align: 'center' });
    pdf.text('Tel: (55) 5365 11 80  |  (55) 5361 31 84', 247, fY - 3, { align: 'center' });
    const barY = fY + 1;
    pdf.setLineWidth(0.6);
    pdf.setDrawColor(141, 198, 63);
    pdf.line(15, barY, 148.5, barY);
    pdf.setDrawColor(0, 160, 227);
    pdf.line(148.5, barY, 282, barY);
    pdf.setFontSize(7.5);
    pdf.setTextColor(148, 163, 184);
    pdf.text(`Página ${pNum}`, 148.5, 203, { align: 'center' });
  }

  private groupByCategory(items: EvidenceItem[]): Record<string, EvidenceItem[]> {
    return items.reduce(
      (acc, item) => {
        const cat = this.normalizeCategory(item.category);
        if (!acc[cat]) acc[cat] = [];
        acc[cat].push(item);
        return acc;
      },
      ...
      for (let sectionPageIndex = 0; sectionPageIndex < pages.length; sectionPageIndex++) {
                    ...
                    new TableCell({
                      children: [
                        new Paragraph({
                          ...
                          children: [
                            new TextRun({ text: fullTitle, bold: true, size: 22, color: '000000' }),
                          ],
                        }),
                      ],
                    }),
                    new TableCell({
                      children: [
                        new Paragraph({
                          ...
                          children: [
                            new TextRun({
                              text: this.formatCapturedDate(this.getTodayDateInputValue()),
                              bold: true,
                              size: 22,
                              color: '000000',
                            }),
        ...
        pageChildren.push(
          ...
          new Paragraph({
            ...
            spacing: { before: 150 },
            children: [
              new TextRun({ text: `Página ${globalPageIndex + 1}`, size: 18, color: '4B5563' }),
            ],
          }),
        ...
        sections.push({
          properties: { page: { margin: { top: 540, bottom: 540, left: 540, right: 540 } } },
  ...
  private buildWordHeaderLeft(docx: DocxModule) {
    ...
    return new Paragraph({
      ...
      spacing: { after: 20 },
      children: [
        new TextRun({ text: 'CPICSA', bold: true, color: '00A0E3', size: 40 }),
        new TextRun({
  ...
  private buildWordHeaderRight(docx: DocxModule) {
    ...
    return new Paragraph({
      ...
      spacing: { after: 170 },
      children: [
        new TextRun({ text: 'PLANTA TEMAXCAL No. 2A', bold: true, color: '00A0E3', size: 14 }),
        new TextRun({
            ...
            children: [
              ...
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    children: [
                      new TextRun({ text: `${evidenceIndex + 1}.- `, bold: true, size: 19 }),
                      new TextRun({
                        text: (
                          item.description.trim() || 'SIN DESCRIPCIÓN CAPTURADA.'
                        ).toUpperCase(),
                        size: 19,
                      }),
                    ],
  ...
  private async processEvidenceText(id: string, file: File) {
    this.evidences.update((current) =>
      current.map((item) => (item.id === id ? { ...item, isProcessing: true } : item)),
    );
    try {
      ...
      this.evidences.update((current) =>
        current.map((item) =>
          item.id === id
            ? { ...item, description: result.text || item.description, isProcessing: false }
            : item,
        ),
      );
    } catch {
      this.evidences.update((current) =>
        current.map((item) => (item.id === id ? { ...item, isProcessing: false } : item)),
      );
    }
  }
  ngOnDestroy() {
    for (const url of this.managedPreviewUrls) URL.revokeObjectURL(url);
  ...
  private revokePreviewUrl(url: string) {
    if (!this.managedPreviewUrls.has(url)) return;
  ...
  private formatCapturedDate(value: string) {
    if (!value) return this.formatDateForBadge(this.getTodayDateInputValue());
  ...
  private normalizeCategory(value: string, fallback = 'SIN SECCIÓN') {
    const category = value.trim().toUpperCase();
    return category || fallback;
  }
  private sortCategories(a: string, b: string) {
